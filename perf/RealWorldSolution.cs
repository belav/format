...
namespace Microsoft.CodeAnalysis.Tools.Perf
{
    ...
    public class RealWorldSolution
    {
        ...
        [IterationCleanup]
        public void RealWorldSolutionCleanup() =>
        ...
        private class RealWorldConfig : ManualConfig
        {
            ...
            {
                var job = Job.Dry.WithPlatform(
                        BenchmarkDotNet.Environments.Platform.X64
                    )
                    .WithRuntime(CoreRuntime.Core21)
                    .WithWarmupCount(1)
                    .WithIterationCount(12)
                    .WithOutlierMode(
                        Perfolizer.Mathematics.OutlierDetection.OutlierMode.RemoveAll
                    );
                Add(
